...
if(isset($_SESSION['admin_id'])){
  include "init.php";
   if($_SERVER['REQUEST_METHOD']== 'POST'){
    ...
    $email_unique = uniqueValue('admins', 'email', $email, $id);
    if($email_unique > 0){
    ...
    if (empty($errors_array)) {
        if($passowrd){
          ...
          $stmt = $pdo->prepare("UPDATE admins SET first_name = :zfirst, last_name = :zlast, email = :zemail , password = :zpass
                                  WHERE id = $id");  
          $stmt->execute(array(
            'zfirst' => $first_name,
            'zlast'  => $last_name,
          ...
          echo "<div class='alert alert-success'>Admin updated successfully</div>";
            ...
            $stmt = $pdo->prepare("UPDATE admins SET first_name = :zfirst, last_name = :zlast, email = :zemail
                                    WHERE id = $id");            
            $stmt->execute(array(
                'zfirst' => $first_name,
                'zlast'  => $last_name,
...
<form action="admins_edit.php?adminid=<?php echo $row['id'] ;?>" method="POST">
