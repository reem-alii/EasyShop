...
if(isset($_SESSION['admin_id'])){
  include "init.php";
   if($_SERVER['REQUEST_METHOD']== 'POST'){
    ...
    $email_unique = uniqueValue('users','email', $email);
    if($email_unique > 0){
    ...
    if (empty($errors_array)) {
      ...
      $stmt = $pdo->prepare("INSERT INTO users(first_name, last_name, email, password, reg_status) 
      VALUES(:zfirst, :zlast, :zemail, :zpass, :zreg)");
      $stmt->execute(array(
        'zfirst' => $first_name,
        'zlast'  => $last_name,
        'zemail' => $email,
